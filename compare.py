...
if __name__ == '__main__':
    ...
    def training(rng, num_epochs, params, X0, X1, step_size, optimizer=optimizers.adam, discrete=False):
    ...
    running_time = end - start
    print('training time (coupled): %.5f sec' %running_time)
    start = time.time()
    trained_params_dis, loss_history_dis = training(rng, args.epoch, params, X0, X1, args.step, discrete=True)
    end = time.time()
    running_time = end - start
    print('training time (discrete): %.5f sec' %running_time)
    ...
    running_time = end - start
    print('free energy using trained model (coupled): %f ± %f' %(fe, fe_err))
    print('importance sampling time (coupled): %.5f sec' %running_time)

    start = time.time()
    fe_rng, rng = random.split(rng)
    fe, fe_err, X_syn_dis = free_energy(fe_rng, trained_params_dis, args.samplesize)
    end = time.time()
    running_time = end - start
    print('free energy using trained model (discrete): %f ± %f' %(fe, fe_err))
    print('importance sampling time (discrete): %.5f sec' %running_time)
    ...
    X_syn_dis_show = X_syn_dis.reshape(-1, args.dim)
    ...
    y_dis = jnp.reshape(jnp.array(loss_history_dis), (-1, 2))
    plt.errorbar(jnp.arange(y.shape[0]), y[:, 0], yerr=y[:, 1], marker='o', capsize=8, label='coupled')
