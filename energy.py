...
@partial(vmap, in_axes=(0, None, None)) # checked
...
def make_free_energy(energy_fun, batched_sampler, logp_fun, n, dim, beta):
    def free_energy(rng, params, sample_size):
        ...
        z, z_err = jnp.exp(- beta * energy_fun(x, n, dim) - logp_fun(params, x)).mean(), jnp.exp(- beta * energy_fun(x, n, dim) - logp_fun(params, x)).std() / jnp.sqrt(x.shape[0])
